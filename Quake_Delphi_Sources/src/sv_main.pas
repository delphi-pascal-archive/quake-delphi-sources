...
// directed messages go only to the entity the are targeted on
  ...
  if field_mask and SND_ATTENUATION <> 0 then MSG_WriteByte(@sv.datagram, int(attenuation * 64));
...
begin
  ...
  sprintf(msg, PChar(@pr_strings[sv.edicts.v._message])); // VJ sos
...
// send music
  ...
  MSG_WriteByte(@client._message, int(sv.edicts.v.sounds));
  MSG_WriteByte(@client._message, int(sv.edicts.v.sounds));
...
begin
  client := @svs.clients[clientnum]; // VJ check!
...
//#endif
  ...
  // call the progs to get default spawn parms for the new client
    ...
    for i := 0 to NUM_SPAWN_PARMS - 1 do
      client.spawn_parms[i] := PFloatArray(@pr_global_struct.parm1)[i]; // VJ ???
  end;
...
// send an update
  ...
  //
    ...
    if bits and U_MODEL <> 0 then MSG_WriteShort(msg, int(ent.v.modelindex)); //SV !!
    if bits and U_FRAME <> 0 then MSG_WriteByte(msg, int(ent.v.frame));
    if bits and U_COLORMAP <> 0 then MSG_WriteByte(msg, int(ent.v.colormap));
    if bits and U_SKIN <> 0 then MSG_WriteByte(msg, int(ent.v.skin));
    if bits and U_EFFECTS <> 0 then MSG_WriteByte(msg, int(ent.v.effects));
...
begin
  ...
  for e := 1 to sv.num_edicts - 1 do
  begin
    ent.v.effects := int(ent.v.effects) and (not EF_MUZZLEFLASH);
    ent := NEXT_EDICT(ent)
  end;
end;
...
*)
  ...
  if val <> nil then
    items := int(ent.v.items) or (int(val._float) shl 23)
  else
    items := int(ent.v.items) or (int(pr_global_struct.serverflags) shl 28);
//#endif

  bits := bits or SU_ITEMS;

  if int(ent.v.flags) and FL_ONGROUND <> 0 then
...
// [always sent]  if (bits & SU_ITEMS)
  ...
  if bits and SU_WEAPON <> 0 then
    MSG_WriteByte(msg, SV_ModelIndex(@pr_strings[ent.v.weaponmodel])); // VJ check!
  ...
  begin
    for i := 0 to 31 do
    begin
      if int(ent.v.weapon) and (1 shl i) <> 0 then
      begin
        MSG_WriteByte(msg, i);
        break;
      end;
    ...
    begin
      ...
      begin client := @svs.clients[j];
        ...
        begin
          ...
          MSG_WriteShort(@client._message, int(host_client.edict.v.frags));
        end;
      end;
...
begin
  ...
  //
    ...
    begin
      ...
      svent.baseline.modelindex := SV_ModelIndex(@pr_strings[svent.v.model]); // VJ check!
    end;
  ...
  begin host_client := @svs.clients[i];
    ...
    // call the progs to get default spawn parms for the new client
      ...
      for j := 0 to NUM_SPAWN_PARMS - 1 do
        host_client.spawn_parms[j] := PFloatArray(@pr_global_struct.parm1)[j]; // VJ check!
    end;
  end;
end;
...
begin
  ...
  if hostname.text[0] = #0 then
    ConsoleVars.SetValue('hostname', 'UNNAMED');
...
//
  if coop.value <> 0 then ConsoleVars.SetValue('deathmatch', 0);
...
// allocate server memory
  ...
  sv.reliable_datagram.data := @sv.reliable_datagram_buf[0]; // VJ check!
...
//
  ...
  sv.sound_precache[0] := pr_strings; // VJ check!       !! TODO

  sv.model_precache[0] := pr_strings; // VJ check!
  sv.model_precache[1] := sv.modelname; // VJ check!
...
//
  ...
  ent.free := false;
  ent.v.model := integer(@sv.worldmodel.name[0]) - integer(pr_strings); // VJ check!     TODO
...
var
  i: integer;
begin
  ConsoleVars.RegisterVariable(@sv_maxvelocity);
  ConsoleVars.RegisterVariable(@sv_gravity);
  ConsoleVars.RegisterVariable(@sv_friction);
  ConsoleVars.RegisterVariable(@sv_edgefriction);
  ConsoleVars.RegisterVariable(@sv_stopspeed);
  ConsoleVars.RegisterVariable(@sv_maxspeed);
  ConsoleVars.RegisterVariable(@sv_accelerate);
  ConsoleVars.RegisterVariable(@sv_idealpitchscale);
  ConsoleVars.RegisterVariable(@sv_aim);
  ConsoleVars.RegisterVariable(@sv_nostep);
