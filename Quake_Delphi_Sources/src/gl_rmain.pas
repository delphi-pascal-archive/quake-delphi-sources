...
//  verts := verts += posenum * paliashdr->poseverts;
  inc(verts, posenum * paliashdr.poseverts); // VJ SOS
...
begin
  ...
  begin
    ...
    pose := pose + int(cl.time / interval) mod numposes;
  end;
...
// VJ SOS
  shadedots := @r_avertexnormal_dots[int(e.angles[1] * (SHADEDOT_QUANT / 360.0)) and (SHADEDOT_QUANT - 1)];
...
// add dynamic lights
  for lnum := 0 to MAX_DLIGHTS - 1 do // VJ maybe pointer loop??
  begin
    dl := @cl_dlights[lnum];
    if dl.radius = 0 then
      continue;
    ...
    if add > 0 then
      _ambientlight := _ambientlight + int(add);
  end;
...
// don't allow cheats in multiplayer
  if cl.maxclients > 1 then
    ConsoleVars.SetValue('r_fullbright', '0');
...
begin
  ...
  glLoadIdentity;
  x := r_refdef.vrect.x * glwidth div vid.width; // VJ -> maybe trunc / int() ??
  x2 := (r_refdef.vrect.x + r_refdef.vrect.width) * glwidth div vid.width; // VJ same as above ??
  ...
  if envmap then // VJ -> avoid previous calculation of w, h ??
  begin
    x := 0;
    y2 := 0;
    w := 256;
...
{  colors[0] := 0.0;
  ...
  begin
    ...
    time1 := 0.0; // VJ avoid compiler warning
