...
procedure ED_Write(var f: text; ed: Pedict_t); //VJ SOS file or text???
...
begin
  ...
  begin
    result := @pr_fielddefs[i];
    if strcmp(@pr_strings[result.s_name], name) = 0 then // VJ SOS
      exit;
  end;
  result := nil;
end;
...
begin
  ...
  if def = nil then
    ...
    result := Peval_t(integer(@ed.v) + def.ofs * 4); // VJ SOS
end;
...
begin
  ...
  case _type of
    ev_string:
      begin
        sprintf(line_PR_ValueString, '%s', [PChar(@pr_strings[val._string])]); // VJ SOS
      end;
...
*)
var
  line_PR_UglyValueString: array[0..255] of char; // VJ mayby same buffer with above proc() ??
...
var
  line_PR_GlobalStringNoContents: array[0..127] of char; // VJ check static line_xxxx sizes!
...
begin
  ...
  if def = nil then
    ...
    sprintf(line_PR_GlobalStringNoContents, '%d(%s)', [ofs, PChar(@pr_strings[def.s_name])]); // VJ SOS

  i := strlen(line_PR_GlobalStringNoContents); // VJ mayby external proc() ?? for adding spaces???
  while i < 20 do
  begin
...
begin
  ...
  begin
    d := @pr_fielddefs[i];
    name := @pr_strings[d.s_name]; // VJ SOS, mayby @pr_strings[d.s_name] ??
    if name[strlen(name) - 2] = '_' then // VJ mayby l = strlen(name) and check l >= 2
...
begin
  ...
  begin
    d := @pr_fielddefs[i];
    name := @pr_strings[d.s_name]; // VJ SOS
    if name[strlen(name) - 2] = '_' then // VJ SOS l := strlen(name); if l > 2 then !
      continue; // skip _x, _y, _z vars
...
function ED_NewString(_string: PChar): PChar; // VJ SOS!!!!! (\n) and (\\) convertion!
var
  _new, new_p: PChar;
  i, l: integer;
begin
...
var
  ...
  _type: integer;
begin
  d := pointer(integer(base) + key.ofs * SizeOf(integer)); // VJ SOS

  _type := key._type and (not DEF_SAVEGLOBAL);
  case etype_t(_type) of
    ev_string:
      begin
        Pstring_t(d)^ := ED_NewString(s) - pr_strings; // VJ SOS
      end;
    ...
    ev_function:
      begin
        ...
        Pfunc_t(d)^ := (integer(func) - integer(pr_functions)) div SizeOf(dfunction_t); //Pfunc_t(integer(func) - integer(pr_functions))^; // VJ SOS
      end;
...
// go through all the dictionary pairs
  ...
  begin
    ...
    if strcmp(com_token, 'angle') = 0 then // VJ check
    begin
...
// remove things from different skill levels or deathmatch
    ...
    begin
      if (int(ent.v.spawnflags) and SPAWNFLAG_NOT_DEATHMATCH) <> 0 then // VJ check type casting!
      begin
        ED_Free(ent);
        inc(inhibit);
        continue;
    ...
    else if ((current_skill = 0) and ((int(ent.v.spawnflags) and SPAWNFLAG_NOT_EASY) <> 0)) or // VJ check!
      ((current_skill = 1) and ((int(ent.v.spawnflags) and SPAWNFLAG_NOT_MEDIUM) <> 0)) or
      ((current_skill = 2) and ((int(ent.v.spawnflags) and SPAWNFLAG_NOT_HARD) <> 0)) then
    begin
      ED_Free(ent);
      inc(inhibit);
      continue;
...
// byte swap the header
  for i := 0 to SizeOf(progs^) div 4 - 1 do // VJ SOS (SizeOf)
...
begin
  ...
  ConsoleVars.RegisterVariable(@nomonsters);
  ConsoleVars.RegisterVariable(@gamecfg);
  ConsoleVars.RegisterVariable(@scratch1);
  ConsoleVars.RegisterVariable(@scratch2);
  ConsoleVars.RegisterVariable(@scratch3);
  ConsoleVars.RegisterVariable(@scratch4);
  ConsoleVars.RegisterVariable(@savedgamecfg);
  ConsoleVars.RegisterVariable(@saved1);
  ConsoleVars.RegisterVariable(@saved2);
  ConsoleVars.RegisterVariable(@saved3);
  ConsoleVars.RegisterVariable(@saved4);
end;
