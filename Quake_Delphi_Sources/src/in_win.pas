...
type
  ...
  private
    ...
    procedure Joy_AdvancedUpdate;
    procedure MouseMove(cmd: Pusercmd_t);
    procedure StartupJoystick;
    function ReadJoystick: qboolean;
    procedure JoyMove(cmd: Pusercmd_t);
    procedure StartupMouse;
    procedure RestoreOriginalMouseState;
    procedure SetQuakeMouseState;
  public
    procedure Init;
    procedure Shutdown;
    procedure Commands;
    procedure Move(cmd: Pusercmd_t);
    ...
    procedure MouseEvent(mstate: integer);
  end;
var
  Input: TInput;
...
procedure Joy_AdvancedUpdate_f;
begin
  Input.Joy_AdvancedUpdate;
end;

{ TInput }

procedure TInput.Accumulate;
begin
  if mouseactive then
  begin
...
procedure TInput.ActivateMouse;
begin
...
procedure TInput.DeactivateMouse;
begin
...
begin
  // mouse variables
  ConsoleVars.RegisterVariable(@m_filter);

  // joystick variables
  ConsoleVars.RegisterVariable(@in_joystick);
  ConsoleVars.RegisterVariable(@joy_name);
  ConsoleVars.RegisterVariable(@joy_advanced);
  ConsoleVars.RegisterVariable(@joy_advaxisx);
  ConsoleVars.RegisterVariable(@joy_advaxisy);
  ConsoleVars.RegisterVariable(@joy_advaxisz);
  ConsoleVars.RegisterVariable(@joy_advaxisr);
  ConsoleVars.RegisterVariable(@joy_advaxisu);
  ConsoleVars.RegisterVariable(@joy_advaxisv);
  ConsoleVars.RegisterVariable(@joy_forwardthreshold);
  ConsoleVars.RegisterVariable(@joy_sidethreshold);
  ConsoleVars.RegisterVariable(@joy_pitchthreshold);
  ConsoleVars.RegisterVariable(@joy_yawthreshold);
  ConsoleVars.RegisterVariable(@joy_forwardsensitivity);
  ConsoleVars.RegisterVariable(@joy_sidesensitivity);
  ConsoleVars.RegisterVariable(@joy_pitchsensitivity);
  ConsoleVars.RegisterVariable(@joy_yawsensitivity);
  ConsoleVars.RegisterVariable(@joy_wwhack1);
  ConsoleVars.RegisterVariable(@joy_wwhack2);
...
procedure TInput.JoyMove(cmd: Pusercmd_t);
var
  speed, aspeed: single;
...
begin
  ...
  begin
    ...
    fAxisValue := pdwRawValue[i]^; // VJ mayby add - 32768.0; here
  ...
  // bounds check pitch
  ClientMain.AdjustPitch;
end;

procedure TInput.Joy_AdvancedUpdate;
...
procedure TInput.MouseEvent(mstate: integer);
var
  i: integer;
begin
  if mouseactive then
...
procedure TInput.MouseMove(cmd: Pusercmd_t);
var
  mx, my: integer;
begin
...
//  Con_DPrintf("mx=%d, my=%d\n", mx, my);
  ...
  if boolval(m_filter.value) then
  begin
...
procedure TInput.Move(cmd: Pusercmd_t);
begin
...
function TInput.RawValuePointer(axis: integer): PDWORD;
begin
  case axis of
...
function TInput.ReadJoystick: qboolean;
begin
...
procedure TInput.RestoreOriginalMouseState;
begin
...
procedure TInput.SetQuakeMouseState;
begin
  if mouseactivatetoggle then
    ActivateMouse;
end;

procedure TInput.ShowMouse;
begin
...
procedure TInput.Shutdown;
begin
  DeactivateMouse;
  ShowMouse;
end;

procedure TInput.StartupJoystick;
var
  numdevs: integer;
  jc: JOYCAPS;
  mmr: MMRESULT;
begin
  ...
  mmr := 0; // VJ avoid compiler warning;
...
procedure TInput.UpdateClipCursor;
begin
