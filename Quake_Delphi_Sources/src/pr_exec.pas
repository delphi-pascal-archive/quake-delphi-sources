...
begin
  ...
  begin
    ...
    if f = nil then
      ...
      Con_Printf('%12s : %s'#10, [PChar(@pr_strings[f.s_file]), PChar(@pr_strings[f.s_name])]) // VJ mayby function get_pr_string(offs): PChar;
  end;
end;
...
// make a stack frame
  ...
  begin
    ...
    case st.op of
      ...
      OP_BITAND:
        begin
          c._float := int(a._float) and int(b._float);
        end;

      OP_BITOR:
        begin
          c._float := int(a._float) or int(b._float);
        end;
      ...
      OP_NE_S:
        begin
          c._float := strcmp(@pr_strings[a._string], // VJ mayby intval(boolval(strcmp... ?
...
*)
          ...
          c._int := integer(@PIntegerArray(@ed.v)[b._int]) - integer(sv.edicts); // VJ check!
        end;
...
#endif
*)
          a := Peval_t(@PIntegerArray(@ed.v)[b._int]); // VJ check!
          c._int := a._int;
        end;
