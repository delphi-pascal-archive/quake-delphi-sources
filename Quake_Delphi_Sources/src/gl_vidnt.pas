...
procedure VID_LockBuffer; // VJ remove ?
procedure VID_UnlockBuffer; // VJ remove ?
...
procedure VID_ShiftPalette(palette: PByteArray);
...
// Who knows if it helps, but it probably doesn't hurt
  ...
  vid_modenum := modenum;
  ConsoleVars.SetValue('vid_mode', vid_modenum);
...
begin
  ...
  begin
    if strncmp(tmp, TEXTURE_EXT_STRING, strlen(TEXTURE_EXT_STRING)) = 0 then
      texture_ext := true; // VJ mayby break here ?
    inc(tmp);
  end;
  ...
  begin
    ...
    if not assigned(bindTexFunc) then // VJ check this
...
//  Con_Printf ("%s %s\n", gl_renderer, gl_version);
  ...
  if strncmp(str_gl_renderer, 'PowerVR', 7) = 0 then // VJ SOS was strnicmp
    fullsbardraw := true;

  if strncmp(str_gl_renderer, 'Permedia', 8) = 0 then // VJ SOS was strnicmp
    isPermedia := true;
...
//    table^ := _SHL(255, 24) + r + _SHL(g, 8) + _SHL(b, 16);
    table^ := (255 shl 24) + (r shl 0) + (g shl 8) + (b shl 16); // $FF000000 + r + (g shl 8) + (b shl 16);
//     := v;
    inc(table);
  end;
...
begin
  ...
  begin
    MessageBox(0, 'SetPixelFormat failed', 'Error', MB_OK); // VJ mayby other outproc not messagebox?
    result := false;
    exit;
  end;
...
begin
  ...
  if result = 0 then
    Con_DPrintf('key 0x%02x has no translation'#10, [key]); // VJ check format string
end;
...
begin
  ...
  begin
    ...
    result := temp_VID_GetModeDescription; // VJ mayby @temp_VID_GetModeDescription[0] ?
  end;
...
//  glColorTableEXT := {lp3DFXFUNC(}wglGetProcAddress('glColorTableEXT'){)};
  ...
  newPalette := PByte(@thePalette); // VJ mayby oldPalette, newPalette PInteger and loop i := 0 to 192 ??
  for i := 0 to 255 do
  begin
    ...
    inc(newPalette);
    inc(oldPalette);
    inc(oldPalette); // VJ skip one bit
  end;
...
begin
  ...
  begin
    ...
    inf := f * 255 + 0.5; // VJ check (??? mayby * 256 ????)
    if inf < 0 then
      inf := 0;
    if inf > 255 then
      inf := 255;
...
begin
  ...
  ConsoleVars.RegisterVariable(@vid_mode);
  ConsoleVars.RegisterVariable(@vid_wait);
  ConsoleVars.RegisterVariable(@vid_nopageflip);
  ConsoleVars.RegisterVariable(@_vid_wait_override);
  ConsoleVars.RegisterVariable(@_vid_default_mode);
  ConsoleVars.RegisterVariable(@_vid_default_mode_win);
  ConsoleVars.RegisterVariable(@vid_config_x);
  ConsoleVars.RegisterVariable(@vid_config_y);
  ConsoleVars.RegisterVariable(@vid_stretch_by_2);
  ConsoleVars.RegisterVariable(@_windowed_mouse);
  ConsoleVars.RegisterVariable(@gl_ztrick);
    ...
    begin
      ...
      begin
        if COM_CheckParm('-width') <> 0 then
          ...
          width := 640;
        height := width * 3 div 4; // VJ avoid compiler warning

        if COM_CheckParm('-bpp') <> 0 then // VJ maybe keep COM_CheckParm() values and not call again function COM_CheckParm()
        begin
...
//  vid.fullbright := 256 - LittleLong (*((int *)vid.colormap + 2048));
  vid.fullbright := 256 - LittleLong(PIntegerArray(@vid.colormap)[2048]); // VJ check this !!
...
begin
  ...
  begin
    MainMenu.Print(2 * 8, 36 + 0 * 8, 'Fullscreen Modes (WIDTHxHEIGHTxBPP)');
    ...
    begin
      if modedescs[i].iscur <> 0 then MainMenu.PrintWhite(column, row, modedescs[i].desc)
      else MainMenu.Print(column, row, modedescs[i].desc);
  ...
  MainMenu.Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 2, 'Video modes must be set from the');
  MainMenu.Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 3, 'command line with -width <width>');
  MainMenu.Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 4, 'and -bpp <bits-per-pixel>');
  MainMenu.Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 6, 'Select windowed mode with -window');
end;
