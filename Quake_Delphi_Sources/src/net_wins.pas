...
begin
  ...
  myAddr := PInteger(local.h_addr_list^)^; // VJ check!
...
begin
  ...
  begin
    ...
    ConsoleVars.SetValue('hostname', buff);
  end;
  ...
  begin
    ...
    begin
      ...
      if myAddr = unsigned(INADDR_NONE) then
...
begin
  ...
  dec(winsock_initialized); // VJ mayby external proc the 3 lines below ??
  if winsock_initialized = 0 then
    WSACleanup;
end;
...
var
  ...
  function get_one_int(var p: PChar; const _until: char): integer; // VJ check!
  var
...
begin
  ...
  begin
    ...
    PSockAddrIn(addr).sin_addr.s_addr := PInteger(hostentry.h_addr_list^)^; // VJ check!
