...
type
  ...
  public
    function EntityNum(num: integer): Pentity_t;
    procedure ParseStartSoundPacket;
    procedure KeepaliveMessage;
    procedure ParseServerInfo;
    procedure ParseUpdate(bits: integer);
    procedure ParseBaseline(ent: Pentity_t);
    procedure ParseClientdata(bits: integer);
    procedure NewTranslation(slot: integer);
    procedure ParseStatic;
    procedure ParseStaticSound;
    procedure ParseServerMessage;
  end;
var
  Parser: TParser;
...
function TParser.EntityNum(num: integer): Pentity_t;
begin
...
procedure TParser.KeepaliveMessage;
var
  time: single;
  ret: integer;
  old: sizebuf_t;
...
procedure TParser.NewTranslation(slot: integer);
var
  i, j: integer;
  top, bottom: integer;
...
procedure TParser.ParseBaseline(ent: Pentity_t);
var
  i: integer;
begin
...
procedure TParser.ParseClientdata(bits: integer);
var
  i, j: integer;
begin
...
procedure TParser.ParseServerInfo;
var
  str: PChar;
  i: integer;
...
procedure TParser.ParseServerMessage;
var
  cmd: integer;
  i: integer;
begin
...
//#endif
      ...
      svc_cdtrack:
        begin
          ...
          if (cls.demoplayback or cls.demorecording) and (cls.forcetrack <> -1)
            then CDAudio.Play(byte(cls.forcetrack), true)
          else CDAudio.Play(byte(cl.cdtrack), true);
        end;
...
procedure TParser.ParseStartSoundPacket;
var
  pos: TVector3f;
  channel, ent: integer;
  sound_num: integer;
...
procedure TParser.ParseStaticSound;
var
  org: TVector3f;
...
procedure TParser.ParseUpdate(bits: integer);
var
  i: integer;
  model: PBSPModelFile;
  modnum: integer;
...
begin
  ...
  for i := 0 to 15 do
    if bits and {_SHL}(1 shl i) <> 0 then
      inc(bitcounts[i]);
