...
begin
  ...
  print('host:    %s'#10, [ConsoleVars.VariableString('hostname')]);
  ...
  client := @svs.clients[0]; // VJ check this
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        hours := 0;
      print('#%-2u %-16.16s  %3d  %2d:%02d:%02d'#10, // VJ SOS
...
begin
  ...
  sv_player.v.flags := int(sv_player.v.flags) xor FL_GODMODE; // VJ check xor
  if int(sv_player.v.flags) and FL_GODMODE = 0 then
...
begin
  ...
  sv_player.v.flags := int(sv_player.v.flags) xor FL_NOTARGET; // VJ check xor
  if (int(sv_player.v.flags) and FL_NOTARGET) = 0 then
...
(*
  ...
  {
    strcpy (_startspot, Cmd_Argv_f(2));
...
begin
  ...
  strcpy(name, Cmd_Argv_f(1));
  ClientMain.EstablishConnection(name);
  Host_Reconnect_f;
end;
...
// this silliness is so we can load 1.06 save files, which have float skill values
  ...
  ConsoleVars.SetValue('skill', current_skill);
...
// load the edicts out of the savegame file
  ...
  if cls.state <> ca_dedicated then
  begin
    ClientMain.EstablishConnection('local');
    Host_Reconnect_f;
  end;
end;
...
begin
  ...
  begin
    if Q_strcmp(cl_name.text, newName) = 0 then
      exit;
    ConsoleVars.SetValue('_cl_name', newName);
  ...
  host_client.edict.v.netname := integer(@host_client.name) - integer(pr_strings); // VJ SOS
...
begin
  ...
  Con_Printf('Exe: %s %s'#10, [__TIME__, __DATE__]); // VJ change this, how???
end;
...
void Host_Please_f (void)
{
  ...
  if ((Cmd_Argc_f () == 3) && Q_strcmp(Cmd_Argv_f(1), "#") == 0)
  {
    j = Q_atof(Cmd_Argv_f(2)) - 1;
  ...
  for (j=0, cl = svs.clients ; j<svs.maxclients ; j++, cl++)
  {
    if (!cl->active)
      continue;
    if (Q_strcasecmp(cl->name, Cmd_Argv_f(1)) == 0)
    {
      if (cl->privileged)
      {
        cl->privileged = false;
...
// turn on color set 1
  if not fromServer then
    sprintf(text, '%s%s: ', [Chr(1), save.name]) // VJ check formating string!
  else
    sprintf(text, '%s<%s> ', [Chr(1), hostname.text]); // VJ check formating string!
...
begin
  if Cmd_Argc_f = 1 then
  begin
    Con_Printf('"color" is "%d %d"'#10, [(int(cl_color.value) shr 4), int(cl_color.value) and $0F]);
  ...
  top := top and 15; // VJ mayby same proc inside func to adjust top & bottom
  if top > 13 then
    top := 13;
  ...
  if cmd_source = src_command then
  begin
    ConsoleVars.SetValue('_cl_color', playercolor);
    if cls.state = ca_connected then
      Cmd_ForwardToServer;
    exit;
  end;
...
begin
  ...
  begin
    ...
    nname := PChar(@pr_strings[sv_player.v.netname]); // VJ check!
    if sv.paused then
...
// run the entrance script
  ...
  begin
    ...
    ent.v.netname := integer(@host_client.name) - integer(pr_strings); // VJ check
...
//
  ...
  MSG_WriteLong(@host_client._message, int(pr_global_struct.total_secrets));

  MSG_WriteByte(@host_client._message, svc_updatestat);
  MSG_WriteByte(@host_client._message, STAT_TOTALMONSTERS);
  MSG_WriteLong(@host_client._message, int(pr_global_struct.total_monsters));

  MSG_WriteByte(@host_client._message, svc_updatestat);
  MSG_WriteByte(@host_client._message, STAT_SECRETS);
  MSG_WriteLong(@host_client._message, int(pr_global_struct.found_secrets));

  MSG_WriteByte(@host_client._message, svc_updatestat);
  MSG_WriteByte(@host_client._message, STAT_MONSTERS);
  MSG_WriteLong(@host_client._message, int(pr_global_struct.killed_monsters));
...
begin
  ...
  begin
    host_client := @svs.clients[0]; // VJ check
    i := 0;
    ...
    '0',
      ...
      begin
        ...
        begin
          ...
          begin
            if t[1] = 'a' then
              sv_player.v.items := int(sv_player.v.items) or HIT_PROXIMITY_GUN
            else
              sv_player.v.items := int(sv_player.v.items) or IT_GRENADE_LAUNCHER;
          end
          else if t[0] = '9' then
            sv_player.v.items := int(sv_player.v.items) or HIT_LASER_CANNON
          else if t[0] = '0' then
            sv_player.v.items := int(sv_player.v.items) or HIT_MJOLNIR
          else if t[0] >= '2' then
            sv_player.v.items := int(sv_player.v.items) or ((IT_SHOTGUN shl (Ord(t[0]) - Ord('2'))));
        end
        else
        begin
          if t[0] >= '2' then
...
begin
  ...
  begin
    e := EDICT_NUM(i);
    if strcmp(@pr_strings[e.v.classname], 'viewthing') = 0 then // VJ SOS
    begin
      result := e;
      exit;
    end;
...
procedure Host_ViewDelta(delta: integer); // Added by VJ
var
  e: Pedict_t;
  m: PBSPModelFile;
begin
