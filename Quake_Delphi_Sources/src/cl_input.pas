...
begin
  ...
  if boolval(b.down[0]) or boolval(b.down[1]) then
    exit;

  if not boolval(b.state and 1) then
    exit;
...
//
  ...
  if boolval(in_attack.state and 3) then
    bits := bits or 1;
  in_attack.state := in_attack.state and (not 2);

  if boolval(in_jump.state and 3) then
    bits := bits or 2;
