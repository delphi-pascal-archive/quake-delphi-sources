...
(*
#ifdef QUAKE2
  if (ent.v.solid = SOLID_BSP and
  (ent.v.angles[0] or ent.v.angles[1] or ent.v.angles[2]) )
  begin  // expand for rotation
    float    max, v;
    int      i;
...
//
  if Boolval(int(ent.v.flags) and FL_ITEM) then
  begin
...
(*
#else
  if ( (t1 >= DIST_EPSILON and t2 >= DIST_EPSILON) or (t2 > t1 and t1 >= 0) )
...
(*#ifdef PARANOID
  if (SV_HullPointContents (sv_hullmodel, node.children[side], @mid) = CONTENTS_SOLID) then
  begin
    Con_Printf ('mid PointInHullSolid'#10);
    Result := false;
    exit;
  end;
#endif*)
...
(*
#ifdef QUAKE2
  // rotate start and _end into the models frame of reference
  if (ent.v.solid = SOLID_BSP and
  (ent.v.angles[0] or ent.v.angles[1] or ent.v.angles[2]) )
  begin
    vec3_t  a;
...
(*
  ...
  begin
    ...
    begin
      ...
      VectorCopy (result.endpos, temp);
      result.endpos[0] := DotProduct (temp, forward);
      result.endpos[1] := -DotProduct (temp, right);
      result.endpos[2] := DotProduct (temp, up);
...
// touch linked edicts
  ...
  // might intersect, so do an exact clip
    ...
    if Boolval(int(touch.v.flags) and FL_MONSTER) then
