...
begin
  ...
  if cls.demonum <> -1 then ClientMain.NextDemo
  else ClientMain.Disconnect;

//  longjmp(host_abortserver, 1); // VJ removed!
end;
...
begin
  ...
  if svs.maxclients > 1 then ConsoleVars.SetValue('deathmatch', 1.0)
  else ConsoleVars.SetValue('deathmatch', 0.0);
end;
...
begin
  ...
  ConsoleVars.RegisterVariable(@host_framerate);
  ConsoleVars.RegisterVariable(@host_speeds);

  ConsoleVars.RegisterVariable(@sys_ticrate);
  ConsoleVars.RegisterVariable(@serverprofile);

  ConsoleVars.RegisterVariable(@fraglimit);
  ConsoleVars.RegisterVariable(@timelimit);
  ConsoleVars.RegisterVariable(@teamplay);
  ConsoleVars.RegisterVariable(@samelevel);
  ConsoleVars.RegisterVariable(@noexit);
  ConsoleVars.RegisterVariable(@skill);
  ConsoleVars.RegisterVariable(@developer);
  ConsoleVars.RegisterVariable(@deathmatch);
  ConsoleVars.RegisterVariable(@coop);

  ConsoleVars.RegisterVariable(@pausable);

  ConsoleVars.RegisterVariable(@temp1);
...
// config.cfg cvars
  if host_initialized and not isDedicated then // VJ check this!
  begin
...
begin
//  if boolval(setjmp(host_abortserver)) then // VJ removed!
...
// update audio
  ...
  if host_speeds.value <> 0 then
  begin
    pass1 := int((time1_Host_Frame - time3_Host_Frame) * 1000);
    time3_Host_Frame := Sys_FloatTime;
    pass2 := int((time2_Host_Frame - time1_Host_Frame) * 1000);
    pass3 := int((time3_Host_Frame - time2_Host_Frame) * 1000);
...
begin
  ...
  m := int((timetotal_Host_Frame * 1000) / timecount_Host_Frame);
...
procedure Host_Init(parms: Pquakeparms_t);
begin
  if standard_quake then minimum_memory := MINIMUM_MEMORY
...
begin
  ...
  begin
//    printf('recursive shutdown'#10); VJ check!!
    exit;
  end;
