...
var
  scr_copytop: integer;
  scr_copyeverything: qboolean; // VJ mayby remove ????
...
var
  scr_conlines: single; // lines of console to display // VJ mayby integer??
...
// the finale prints the characters one at a time
  ...
  // scan the width of the line
    l := 0;
    while (l < 40) and not (start[l] in [#0, #10]) do // VJ check this
      inc(l);
...
// bound viewsize
  if scr_viewsize.value < 30 then ConsoleVars.SetValue('viewsize', '30');
  if scr_viewsize.value > 120 then ConsoleVars.SetValue('viewsize', '120');

// bound field of view
  if scr_fov.value < 10 then ConsoleVars.SetValue('fov', '10');
  if scr_fov.value > 170 then ConsoleVars.SetValue('fov', '170');

// intermission is always full screen
  if cl.intermission <> 0 then size := 120
...
begin
  ConsoleVars.SetValue('viewsize', scr_viewsize.value + 10);
...
begin
  ...
  ConsoleVars.RegisterVariable(@scr_fov);
  ConsoleVars.RegisterVariable(@scr_viewsize);
  ConsoleVars.RegisterVariable(@scr_conspeed);
  ConsoleVars.RegisterVariable(@scr_showram);
  ConsoleVars.RegisterVariable(@scr_showturtle);
  ConsoleVars.RegisterVariable(@scr_showpause);
  ConsoleVars.RegisterVariable(@scr_centertime);
  ConsoleVars.RegisterVariable(@scr_printspeed);
  ConsoleVars.RegisterVariable(@gl_triplebuffer);
...
procedure SCR_DrawRam;
begin
  if not boolval(scr_showram.value) then
    exit;
...
procedure SCR_DrawTurtle;
begin
  if not boolval(scr_showturtle.value) then
    exit;
...
//
  ...
  while i <= 99 do
  begin
    pcxname[5] := Chr((i div 10) + Ord('0'));
    pcxname[6] := Chr((i mod 10) + Ord('0'));
  ...
  buffer[13] := glwidth div 256;
  buffer[14] := glheight and 255;
  buffer[15] := glheight div 256;
