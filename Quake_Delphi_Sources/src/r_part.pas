...
begin
  ...
  if avelocities[0][0] = 0 then // VJ check initialization!
  begin
...
{$I-}
    for i := 0 to 2 do
      org[i] := read_float(f); // VJ decimal separators ???
{$I+}
    if IOResult <> 0 then
      break;
    inc(c);
...
begin
  ...
  begin
    ...
    begin
      p._type := pt_explode2;
      for j := 0 to 2 do // VJ mayby call this loop once, see above, same loop.
      begin
        p.org[j] := org[j] + rand mod 32 - 16;
        p.vel[j] := rand mod 512 - 256;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for j := 0 to 2 do
      begin
        p.org[j] := org[j] + rand mod 32 - 16; // VJ mayby special proc to handle this!
        p.vel[j] := rand mod 512 - 256;
      end;
    end
    else
...
//#ifdef GLQUAKE
    ...
    if scale < 20 then
      ...
      scale := 1 + scale * 0.004;
    glColor3ubv(PGLubyte(@d_8to24table[int(p.color)])); // VJ check!, also check int()
    glTexCoord2f(0, 0);
    glVertex3fv(@p.org);
    glTexCoord2f(1, 0);
