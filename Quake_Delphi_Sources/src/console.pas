...
begin
  if con_text <> nil then
    Q_memset(con_text, Ord(' '), CON_TEXTSIZE);
end;
...
//
  ConsoleVars.RegisterVariable(@con_notifytime);
...
begin
  con_x := 0;
  inc(con_current);
  Q_memset(@con_text[(con_current mod con_totallines) * con_linewidth], Ord(' '), con_linewidth);
end;
...
var
  ...
  c: char;
begin
  sprintf(data_Con_DebugLog, PChar(fmt + #10#0), Args); // VJ check!
...
//      SCR_UpdateScreen;                         // SibVRV : commented ... because i unlike wait ...
...
// add the cursor frame
  text[key_linepos] := Chr(10 + int(realtime * con_cursorspeed) and 1);
...
begin
  ...
  begin
    ...
    Draw_Character(((x + 5) shl 3), v, 10 + int(realtime * con_cursorspeed) and 1);
    inc(v, 8);
  end;
...
// draw the text
  ...
  y := lines - 16 - {_SHL}(rows shl 3); // may start slightly negative
