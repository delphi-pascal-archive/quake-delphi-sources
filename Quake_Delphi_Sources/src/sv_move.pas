...
*)
var
  c_yes: integer = 0; // VJ mayby remove...
  c_no: integer = 0;
...
// flying monsters don't step up
  if boolean(int(ent.v.flags) and (FL_SWIM or FL_FLY)) then
  begin
  // try one move with vertical motion, then one without
    ...
    begin
      ...
      begin
        if ((int(ent.v.flags) and FL_SWIM) <> 0) and (SV_PointContents(@trace.endpos[0]) = CONTENTS_EMPTY) then
        begin
...
// push down from a step height above the wished position
  ...
  // if monster had the ground pulled out, go ahead and fall
    if int(ent.v.flags) and FL_PARTIALGROUND <> 0 then
    begin
...
//    Con_Printf ("back on ground\n");
    ent.v.flags := int(ent.v.flags) and (not FL_PARTIALGROUND);
  end;
...
begin
  ...
  move[0] := cos(_yaw) * dist; // VJ mayby SinCos ?
...
//  Con_Printf ("SV_FixCheckBottom\n");
  ...
  ent.v.flags := int(ent.v.flags) or FL_PARTIALGROUND;
end;
