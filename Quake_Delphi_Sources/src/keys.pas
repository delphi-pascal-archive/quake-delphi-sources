...
const
  CHARBUFFERLENGTH = 32; // VJ make this bigger??
...
begin
  if key = K_ENTER then
  begin
    Cbuf_AddText(@key_lines[edit_line][1]); // skip the > // VJ check!
    Cbuf_AddText(#10);
  ...
  begin // command completion
    cmd := Cmd_CompleteCommand(@key_lines[edit_line][1]); // VJ check!
    if cmd = nil then
      cmd := ConsoleVars.CompleteVariable(@key_lines[edit_line][1]);
    if cmd <> nil then
    begin
  ...
  begin
    if key_linepos > 1 then
      dec(key_linepos); // VJ mayby add PlaySound(!!!) ??
    exit;
  end;
  ...
  begin
    ...
    until not ((history_line <> edit_line) and not boolval(key_lines[history_line][1])); // VJ SOS
