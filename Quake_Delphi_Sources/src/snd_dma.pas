...
procedure S_StopAllSoundsC; // VJ remove ?
...
procedure S_BeginPrecaching; // VJ remove ?
procedure S_EndPrecaching; // VJ remove ?
...
begin
  ...
  Con_Printf('0x%x dma buffer'#10, [shm.buffer]); // VJ check hex sprintf!
...
begin
  ...
  ConsoleVars.RegisterVariable(@nosound);
  ConsoleVars.RegisterVariable(@volume);
  ConsoleVars.RegisterVariable(@precache);
  ConsoleVars.RegisterVariable(@loadas8bit);
  ConsoleVars.RegisterVariable(@bgmvolume);
  ConsoleVars.RegisterVariable(@bgmbuffer);
  ConsoleVars.RegisterVariable(@ambient_level);
  ConsoleVars.RegisterVariable(@ambient_fade);
  ConsoleVars.RegisterVariable(@snd_noextraupdate);
  ConsoleVars.RegisterVariable(@snd_show);
  ConsoleVars.RegisterVariable(@_snd_mixahead);

  if host_parms.memsize < $800000 then
  begin
    ConsoleVars.SetValue('loadas8bit', '1');
...
// a bit to keep it from just making the first one louder
  ...
  begin
    ...
    begin
      ...
      begin
        skip := rand mod int(0.1 * shm.speed);
...
begin
  ...
  begin
    ...
    Q_memset(pData, clear, shm.samples * shm.samplebits div 8);

    pDSBuf.Unlock(pData, dwSize, nil, 0);

  end
  else
    Q_memset(shm.buffer, clear, shm.samples * shm.samplebits div 8);
end;
...
// calc ambient sound levels
  ...
  // don't adjust volume too fast
    ...
    begin
      chan.master_vol := chan.master_vol + int(host_frametime * ambient_fade.value);
      if chan.master_vol > vol then
...
procedure S_Update_;
var
  endtime: integer; // VJ was unsigned;
  samps: integer;
  dwStatus: DWORD;
begin
  if not sound_started or (snd_blocked > 0) then
...
// mix ahead of current position
  endtime := int(soundtime + _snd_mixahead.value * shm.speed);
...
procedure S_ClearPrecache; // VJ remove ?
begin
end;
