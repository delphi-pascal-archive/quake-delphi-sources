...
procedure Q_memset(dest: pointer; fill: integer; count: integer);
procedure Q_memcpy(dest: pointer; src: Pointer; count: integer);
...
var
  com_argc: integer;
  com_argv: PArgvArray; //array[0..MAX_NUM_ARGVS + NUM_SAFE_ARGVS] of PChar; // VJ ?? was char **
...
// VJ: needs optimization ???

procedure Q_memset(dest: pointer; fill: integer; count: integer);
var
  i: integer;
begin
  if ((integer(dest) or count) and 3) = 0 then
  begin
    count := count div 4;
    fill := fill or (fill shl 8) or (fill shl 16) or (fill shl 24);
    for i := 0 to count - 1 do
      PIntegerArray(dest)[i] := fill;
  end
  else
    for i := 0 to count - 1 do
      PByteArray(dest)[i] := fill;
end;

// VJ: needs optimization ??? (pointers in loop)

procedure Q_memcpy(dest: pointer; src: Pointer; count: integer);
var
  i: integer;
begin
  if (integer(dest) or integer(src) or integer(count) and 3) = 0 then
  begin
    count := count div 4;
    for i := 0 to count - 1 do
      PIntegerArray(dest)[i] := PIntegerArray(src)[i];
  end
  else
    for i := 0 to count - 1 do
      PByteArray(dest)[i] := PByteArray(src)[i];
end;

// VJ: needs optimization ??? (pointers in loop, 4 byte alignment check)
...
begin
  ...
  if buf.data[buf.cursize - 1] <> 0 then
    ...
    Q_memcpy(pointer(integer(SZ_GetSpace(buf, len - 1)) - 1), data, len); // write over trailing 0
end;
...
begin
  ...
  while pathname^ <> #0 do
  begin
    if pathname^ = '/' then // VJ mayby check and '\'
...
//      Sys_Error('You must have the registered version to use modified games'); // VJ !!!
//    exit;
  end;
{
  ...
  ConsoleVars.SetValue('cmdline', com_cmdline);
  ConsoleVars.SetValue('registered', '1');
  static_registered := 1;
...
// set the byte swapping variables in a portable manner
  ...
  ConsoleVars.RegisterVariable(@registered);
  ConsoleVars.RegisterVariable(@cmdline);
...
//
  ...
  begin
    com_modified := true;
    com_searchpaths := nil;
    while i < com_argc - 1 do // VJ, mayby while i < com_argc do
    begin
      inc(i);
