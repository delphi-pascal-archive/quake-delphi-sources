...
procedure Mod_Init;
begin
  ConsoleVars.RegisterVariable(@gl_subdivide_size);
...
*)
var
  decompressed_Mod_DecompressVis: array[0..MAX_MAP_LEAFS div 8 - 1] of byte;
...
begin
  ...
  _out := @decompressed_Mod_DecompressVis[0];
...
*)
  ...
  begin // no vis info, so make all visible
    ...
    result := @decompressed_Mod_DecompressVis;
    exit;
  end;
  ...
  until integer(_out) - integer(@decompressed_Mod_DecompressVis[0]) >= row;
//#endif

  result := @decompressed_Mod_DecompressVis[0];
end;
...
begin
  ...
  inc(pin_intervals, numframes); // VJ this should work!

  ptemp := pointer(pin_intervals);

  for i := 0 to numframes - 1 do
  begin
    poseverts[posenum] := Ptrivertx_t(integer(ptemp) + SizeOf(daliasframe_t)); // VJ -> check this!
    inc(posenum);

    ptemp := Ptrivertx_t {pointer}((integer(ptemp) + SizeOf(daliasframe_t)) + SizeOf(trivertx_t) * pheader.numverts); // VJ -> check this
  end;
...
begin
  ...
  //    if (!strcmp(loadmodel->name,"progs/player.mdl")) {
      ...
      pheader.texels[i] := integer(texels) - integer(pheader); // VJ -> check this!
...
//
  ...
  pframetype := Pdspriteframetype_t(integer(pin) + SizeOf(dsprite_t)); // VJ -> check this
