...
// fast special case
    for i := 0 to outcount - 1 do
      signed_char(sc.data[i]) := data[i] - 128; // VJ ???
  end
  else
  begin
// general case
    ...
    begin
      ...
      if inwidth = 2 then
        ...
        sample := ((data[srcsample] - 128) shl 8); // VJ check!
      if sc.width = 2 then
...
var
  ...
  stepscale: single;
  sc: Psfxcache_t;
  stackbuf: array[0..1 * 1024 - 1] of byte; // avoid dirtying the cache heap // VJ ???
begin
...
//  Con_Printf ("loading %s\n",namebuffer);
  ...
  data := COM_LoadStackFile(namebuffer, @stackbuf, SizeOf(stackbuf)); // VJ check stackbuf
...
function GetLittleShort: short; // VJ check shifts, negative values???
begin
  result := data_p^;
  inc(data_p);
...
function GetLittleLong: integer; // VJ check shifts, negative values???
begin
  result := data_p^;
  inc(data_p);
  result := result + data_p^ * 256;
  ...
  // if the next chunk is a LIST chunk, look for a cue length marker
    ...
    begin
      ...
      begin // this is not a proper parse, but it works with cooledit...
        ...
        result.samples := result.loopstart + i; // VJ pointers ???
...
// find data chunk
  ...
  result.dataofs := integer(data_p) - integer(wav); // VJ check!
