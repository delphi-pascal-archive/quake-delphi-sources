...
type
  glRect_t = record
    l, t, w, h: byte; // VJ was unsigned char
  end;
  PglRect_t = ^glRect_t;
    ...
    begin
      ...
      begin
        ...
        if sd < 0 then
          sd := -sd;
        if sd > td then
          dist := sd + (td / 2) // VJ mayby not div, just /
        else
          dist := td + (sd / 2);
        if dist < minlight then
          blocklights[t * smax + s] := blocklights[t * smax + s] + uint((rad - dist) * 256);
      end;
    end;
  end;
end;
...
begin
  ...
  reletive := int(cl.time * 10) mod base.anim_total;
...
begin
  ...
  begin
    ...
    // VJ mayby optimize this -> eg tmp := 8 * sin(v[2] * 0.05 + realtime)
    nv[0] := v[0] + 8 * sin(v[1] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
    nv[1] := v[1] + 8 * sin(v[0] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
    nv[2] := v[2];
...
begin
  ...
  begin
    ...
    // VJ mayby optimize this -> tmp := 8 * sin(v[2]*0.05+realtime) ??
    nv[0] := v[0] + 8 * sin(v[1] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
    nv[1] := v[1] + 8 * sin(v[0] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
    nv[2] := v[2];
...
begin
  ...
  if gl_lightmap_format = GL_RGBA then glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
  else if gl_lightmap_format = GL_RGB then begin
    glBlendFunc(GL_ZERO, GL_ONE_MINUS_SRC_COLOR);
  end
  else if gl_lightmap_format = GL_LUMINANCE then glBlendFunc(GL_ZERO, GL_ONE_MINUS_SRC_COLOR)
...
var
  ...
  t: Ptexture_t;
begin
  if (r_wateralpha.value = 1.0) and boolval(gl_texsort.value) then
    exit;
  ...
  if not boolval(gl_texsort.value) then
  begin
  ...
  begin
    ...
    begin
      ...
      if s = nil then
        continue;
      if not boolval(s.flags and SURF_DRAWTURB) then
        continue;
...
// instanced model
  ...
  begin
    ...
    begin
      ...
      nd := clmodel.nodes;
      inc(nd, clmodel.hulls[0].firstclipnode); // VJ mayby outside loop ?
...
{$IFDEF DEBUG}
  if f = NULLFILE then
    f := fopen('c:\debugd.txt', 'w');
  fprintf(f, 'contents: %d, visframe: %d'#10, [node.contents, node.visframe]);
{$ENDIF}
...
// draw stuff
  ...
  begin
    ...
    begin
      ...
      if (surf.flags and SURF_UNDERWATER = 0) and ((dot < 0) xor boolval(surf.flags and SURF_PLANEBACK)) then // VJ check this!! SOS
...
var
  ...
  node: Pmnode_t;
  i: integer;
  solid: array[0..4095] of byte; // VJ mayby static?
begin
...
var
  i, j: integer;
  m: PBSPModelFile;
//  extern qboolean isPermedia; // VJ check where is declared
begin
...
//  gl_lightmap_format := DEFAULT_LIGHTMAP_FORMAT;// GL_LUMINANCE;
  ...
  begin
    ...
    begin
      GL_CreateSurfaceLightmap(@m.surfaces[i]); // VJ SOS
