...
// see if any solid entities are inside the final position
  ...
  begin
    if check.free then
      goto continue1;
    if int(check.v.movetype) in [MOVETYPE_PUSH, MOVETYPE_NONE, MOVETYPE_NOCLIP] then
(*
#ifdef QUAKE2
...
//
  ...
  begin
    ...
    begin
      Con_Printf('Got a NaN velocity on %s'#10, [PChar(@pr_strings[ent.v.classname])]); // VJ check
      ent.v.velocity[i] := 0;
    end;
    if IS_NAN(ent.v.origin[i]) then
    begin
      Con_Printf('Got a NaN origin on %s'#10, [PChar(@pr_strings[ent.v.classname])]); // VJ check
      ent.v.origin[i] := 0;
    end;
...
*)
  ...
  // if the entity is standing on the pusher, it will definately be moved
    if not (((int(check.v.flags) and FL_ONGROUND) <> 0) and
  ...
  // remove the onground flag for non-players
    if (check.v.movetype <> MOVETYPE_WALK) then
      check.v.flags := int(check.v.flags) and (not FL_ONGROUND);
  ...
  // if it is still inside the pusher, block
    ...
    begin // fail the move
      if check.v.mins[0] = check.v.maxs[0] then
        goto continue1;
      if int(check.v.solid) in [SOLID_NOT, SOLID_TRIGGER] then
      begin // corpse
        check.v.mins[0] := 0;
        check.v.mins[1] := 0;
...
*)
const
  STEPSIZE = 18; // VJ declared also in sv_move, remove 1 definition ?
...
//
  oldonground := int(ent.v.flags) and FL_ONGROUND;
  ent.v.flags := int(ent.v.flags) and (not FL_ONGROUND);
  ...
  if int(sv_player.v.flags) and FL_WATERJUMP <> 0 then
    exit;
  ...
  case int(ent.v.movetype) of
    ...
    MOVETYPE_WALK:
      begin
        if not SV_RunThink(ent) then
          exit;
        if (not SV_CheckWater(ent)) and (not boolval(int(ent.v.flags) and FL_WATERJUMP)) then
          SV_AddGravity(ent);
        SV_CheckStuck(ent);
(*
#ifdef QUAKE2
...
// freefall if not onground
  if (int(ent.v.flags) and (FL_ONGROUND or FL_FLY or FL_SWIM)) = 0 then
  begin
    ...
    if int(ent.v.flags) and FL_ONGROUND <> 0 then // just hit ground
    begin
      if hitsound then
        SV_StartSound(ent, 0, 'demon/dland2.wav', 255, 1);
    end;
...
*)
        ...
        SV_Physics_Step(ent)
      else if int(ent.v.movetype) in [MOVETYPE_TOSS, MOVETYPE_BOUNCE, MOVETYPE_FLY, MOVETYPE_FLYMISSILE] then
(*
#ifdef QUAKE2
    || ent.v.movetype = MOVETYPE_BOUNCEMISSILE
#endif
...
//  particle_t  *p;
  ...
  while (1)
  {
    ...
    trace := SV_MoveEdict(tent.v.origin, tent.v.mins, tent.v.maxs, end, MOVE_NORMAL, tent);
