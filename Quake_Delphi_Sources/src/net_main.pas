...
(*
#ifdef IDGODS
    idgods: cvar_t = (name: 'idgods'; _string: '0');
#endif
*)
...
begin
  ...
  begin
    net_driverlevel := i; // VJ must do this ???
...
begin
  ...
  svs.maxclients := n;
  if n = 1 then ConsoleVars.SetValue('deathmatch', '0')
  else ConsoleVars.SetValue('deathmatch', '1');
end;
...
begin
  ...
  slistSilent := boolval(host); // VJ check
  NET_Slist_f;
...
function NET_CanSendMessage(sock: Pqsocket_t): qboolean;
begin
  if sock = nil then // VJ mayby procvalidsock(sock): qboolean ?
  begin
    result := false;
    exit;
  end;
...
begin
  ...
  ConsoleVars.RegisterVariable(@net_messagetimeout);
  ConsoleVars.RegisterVariable(@hostname);
  ConsoleVars.RegisterVariable(@config_com_port);
  ConsoleVars.RegisterVariable(@config_com_irq);
  ConsoleVars.RegisterVariable(@config_com_baud);
  ConsoleVars.RegisterVariable(@config_com_modem);
  ConsoleVars.RegisterVariable(@config_modem_dialtype);
  ConsoleVars.RegisterVariable(@config_modem_clear);
  ConsoleVars.RegisterVariable(@config_modem_init);
  ConsoleVars.RegisterVariable(@config_modem_hangup);
(*
#ifdef IDGODS
...
begin
  ...
  begin
    ...
    begin
      serialAvailable := false; // VJ not serial support!!
{      useModem := config_com_modem.value = 1.0;
      SetComPortConfig(0, int(config_com_port.value), int(config_com_irq.value), int(config_com_baud.value), useModem);
      SetModemConfig(0, config_modem_dialtype._string, config_modem_clear._string, config_modem_init._string, config_modem_hangup._string);}
    end;
    configRestored := true;
  end;
