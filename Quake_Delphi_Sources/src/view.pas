...
begin
  ...
  for i := 0 to 255 do
  begin
    inf := int(255 * fpow((i + 0.5) / 255.5, g) + 0.5);
    if inf < 0 then
      inf := 0
...
begin
  ...
  cl.cshifts[CSHIFT_DAMAGE].percent := cl.cshifts[CSHIFT_DAMAGE].percent + int(3 * count);
...
// drop the damage value
  cl.cshifts[CSHIFT_DAMAGE].percent := cl.cshifts[CSHIFT_DAMAGE].percent - int(host_frametime * 150);
  if cl.cshifts[CSHIFT_DAMAGE].percent <= 0 then
    cl.cshifts[CSHIFT_DAMAGE].percent := 0;

// drop the bonus value
  cl.cshifts[CSHIFT_BONUS].percent := cl.cshifts[CSHIFT_BONUS].percent - int(host_frametime * 100);
...
//      exit; // VJ added handling when in god mod

  if v_dmg_time > 0 then // VJ here corretc the god mod bug inside water ???
  begin
    r_refdef.viewangles[ROLL] := r_refdef.viewangles[ROLL] +
...
(*
#if 0
  if (cl.model_precache[cl.stats[STAT_WEAPON]] and strcmp (cl.model_precache[cl.stats[STAT_WEAPON]].name,  'progs/v_shot2.mdl'))
#endif
*)
...
// don't allow cheats in multiplayer
  ...
  begin
    ConsoleVars.SetValue('scr_ofsx', '0');
    ConsoleVars.SetValue('scr_ofsy', '0');
    ConsoleVars.SetValue('scr_ofsz', '0');
  end;
...
begin
  ...
  ConsoleVars.RegisterVariable(@lcd_x);
  ConsoleVars.RegisterVariable(@lcd_yaw);

  ConsoleVars.RegisterVariable(@v_centermove);
  ConsoleVars.RegisterVariable(@v_centerspeed);

  ConsoleVars.RegisterVariable(@v_iyaw_cycle);
  ConsoleVars.RegisterVariable(@v_iroll_cycle);
  ConsoleVars.RegisterVariable(@v_ipitch_cycle);
  ConsoleVars.RegisterVariable(@v_iyaw_level);
  ConsoleVars.RegisterVariable(@v_iroll_level);
  ConsoleVars.RegisterVariable(@v_ipitch_level);

  ConsoleVars.RegisterVariable(@v_idlescale);
  ConsoleVars.RegisterVariable(@crosshair);
  ConsoleVars.RegisterVariable(@cl_crossx);
  ConsoleVars.RegisterVariable(@cl_crossy);
  ConsoleVars.RegisterVariable(@gl_cshiftpercent);

  ConsoleVars.RegisterVariable(@scr_ofsx);
  ConsoleVars.RegisterVariable(@scr_ofsy);
  ConsoleVars.RegisterVariable(@scr_ofsz);
  ConsoleVars.RegisterVariable(@cl_rollspeed);
  ConsoleVars.RegisterVariable(@cl_rollangle);
  ConsoleVars.RegisterVariable(@cl_bob);
  ConsoleVars.RegisterVariable(@cl_bobcycle);
  ConsoleVars.RegisterVariable(@cl_bobup);

  ConsoleVars.RegisterVariable(@v_kicktime);
  ConsoleVars.RegisterVariable(@v_kickroll);
  ConsoleVars.RegisterVariable(@v_kickpitch);

  BuildGammaTable(1.0); // no gamma yet
  ConsoleVars.RegisterVariable(@v_gamma);
end;
