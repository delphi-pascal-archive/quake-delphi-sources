...
// world
var
  angles: PVector3f; //PFloatArray; // VJ mayby Pvec3_t?
  origin: PVector3f; //PFloatArray; // VJ mayby Pvec3_t?
  velocity: PVector3f; //PFloatArray; // VJ mayby Pvec3_t?
...
begin
  if (int(sv_player.v.flags) and FL_ONGROUND) = 0 then
    exit;

  angleval := sv_player.v.angles[YAW] * M_PI * 2 / 360;
  sinval := sin(angleval); // VJ mayby sincos ?
  cosval := cos(angleval);
...
begin
  ...
  begin
    sv_player.v.flags := int(sv_player.v.flags) and (not FL_WATERJUMP);
...
// hack to not let you back into teleporter
  ...
  if int(sv_player.v.movetype) <> MOVETYPE_WALK then
...
begin
  ...
  onground := int(sv_player.v.flags) and FL_ONGROUND <> 0;
