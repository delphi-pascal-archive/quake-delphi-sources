...
var
  ...
  start: integer;
begin
  ConsoleVars.RegisterVariable(@gl_nobind);
  ConsoleVars.RegisterVariable(@gl_max_size);
  ConsoleVars.RegisterVariable(@gl_picmip);

  // 3dfx can only handle 256 wide textures
  if (Q_strncasecmp(str_gl_renderer, '3dfx', 4) = 0) or
    (strstr(str_gl_renderer, 'Glide') <> nil) then
    ConsoleVars.SetValue('gl_max_size', '256');
  ...
  // it into a texture
  draw_chars := WADFile.GetLumpName('conchars');
